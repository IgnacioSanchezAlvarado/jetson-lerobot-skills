...
> For detailed SSH, WiFi, and Ethernet setup procedures, read #[[file:set-up/02-setup_ssh-guide.md]].
...
- Check provisioning: read `/opt/digital-twin/config.json` → `_provision` block
  ...
  - If missing → run `aws-pai edge provision` and bootstrap the Jetson first (see #[[file:set-up/01-setup_jetson-plan.md]] Phase 4)
...
- For detailed port identification and camera mapping, read #[[file:operations/01-lerobot_commands.md]] (camera wiring table and teleoperation sections)
